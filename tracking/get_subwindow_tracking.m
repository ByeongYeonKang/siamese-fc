% --------------------------------------------------------------------------------------------------------
function [im_patch, im_patch_original, image_coord_roi, score_coord_roi] = get_subwindow_tracking(im, pos, model_sz, original_sz, avg_chans)
...
% -------------------------------------------------------------------------------------------------
    ...
    %roi_score = [max(1,left_pad+1), sz(1)-right_pad, max(1,top_pad+1), sz(2)-bottom_pad, sz(1)];
    %roi_region = [max(1,context_xmin), min(im_sz(2),context_xmax), max(1,context_ymin), min(im_sz(1),context_ymax)];
    score_coord_roi = [max(1,left_pad+1), max(1,top_pad+1), sz(2)-right_pad, sz(1)-bottom_pad];
    image_coord_roi = [context_xmin+left_pad, context_ymin+top_pad, min(context_xmax,im_sz(2)), min(context_ymax,im_sz(1)), sz(1)]; 
